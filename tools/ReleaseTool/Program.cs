...
{
    private static TextWriter _originalOut = null!;
    ...
    private static void Main(string[] args)
    {
        try
        {
            ...
            else
            {
                ...
                var translationName = root.Name.Replace("-master", "").Replace("-main", ""); //todo let user enter it
                ...
                {
                    DoRelease(translationName, root, translationPriorityNumber);
                }
            }
        }
        catch (Exception e)
        ...
        if (_toCleanup.Any())
        {
            ...
            {
                try { info.Delete(); }
                catch { /* don't care about temp files */ }
            }
        }
    ...
    private static void DoRelease(string translationName, DirectoryInfo root, int translationPriorityNumber)
    {
        ...
        {
            Debug.Assert(languageCode != null, nameof(languageCode) + " != null");
            ...
            {
                TagsToSplitterRegexes(Path.Combine(textDir, "Dialogue"));

                if (Directory.GetFiles(textDir, "*.txt", SearchOption.AllDirectories).Any())
                {
    ...
    private static void TagsToSplitterRegexes(string dialogueDir, bool readOnly = false)
    {
        ...
        foreach (var dialogueFile in Directory.EnumerateFiles(dialogueDir, "*.txt"))
        {
            var lines = File.ReadAllLines(dialogueFile).ToList();
            for (var i = 0; i < lines.Count; i++)
            {
                // Only translated files
                var line = lines[i];
                var lineParts = line.Split(new char[] { '=' }, 2, StringSplitOptions.None);
                if (lineParts.Length != 2)
                    continue;

                var orig = lineParts[0];
                var tl = lineParts[1];

                var matchCollection = Regex.Matches(orig, @"\[\w+?\]");
                var uniqueTags = matchCollection.Select(x => x.Value).Distinct().ToList();

                // Convert line with tags to a regex
    ...
    private static Results CleanTranslations(DirectoryInfo dir, DirectoryInfo rootDir)
    {
        ...
        foreach (var file in files)
        {
            ...
            {
                folderPercentage.Total += lines.Count(x => !x.EndsWith("=---") && x.Replace("\\=", "").Contains('='));
                folderPercentage.Translated += lines.Count(x => !x.StartsWith("//") && !x.EndsWith("=---") && x.Replace("\\=", "").Contains('='));
            }
        }
